...
public class SubscriptionDaoImpl extends BaseDaoImpl implements SubscriptionDao {
    ...
    private static final String READ_ACTIVE_BY_VISITOR = "SELECT `id`,`begin_date`,`end_date`,`price` FROM `subscription` WHERE `end_date` >= NOW() AND `begin_date`<= NOW() AND `visitor_id` = ?";
